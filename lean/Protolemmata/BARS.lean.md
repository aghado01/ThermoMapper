...
- **MP-2** `count_of_mean_undercounts` — peak _count_ of the mean undercounts.
...
/-- **MP-2.** Peak _count_ of the mean undercounts: two curves each with two significant maxima
...
strict monotonicity for uniqueness. The span endpoint is _this_ root, so the per-draw reduction
